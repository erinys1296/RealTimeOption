...
def get_ticks_df(symbol):
    ticks = api.ticks(
        ...
        #抓取當天日期
        #若當天為假日，則抓取前一天
        date = datetime.now().strftime('%Y-%m-%d') if datetime.now().weekday() < 5 else (datetime.now() - timedelta(days=2)).strftime('%Y-%m-%d'),
...
Final60Tdata = futuredata.get_future_T_data(60,5)
Final01Tdata = futuredata.get_future_T_data(1,3)
...
def update_left_charts(n, kline_interval):
    ...
    limit_github_url = "https://raw.githubusercontent.com/erinys1296/StockData/erinys1296-patch-4/limit_data.json"
    cost_github_url = "https://raw.githubusercontent.com/erinys1296/StockData/erinys1296-patch-4/cost_data.json"
    response = requests.get(limit_github_url, timeout=30)
    response.raise_for_status()
    json_data = response.json()
    limit_df = pd.DataFrame(json_data['data'])

    response = requests.get(cost_github_url, timeout=30)
    response.raise_for_status()
    json_data = response.json()
    cost_df = pd.DataFrame(json_data['data'])

    #print(limit_df)
    # 檢查 DataFrame 是否為空，以及列數是否符合預期
    if not cost_df.empty and len(cost_df.columns) == 2:
        cost_df.columns = ['Dateonly', 'Cost']
        cost_df['Dateonly'] = pd.to_datetime(cost_df['Dateonly']).dt.date  # 確保 Dateonly 是日期格式
    else:
        print(f"警告: cost_df 為空或欄位數不是 2 (實際: {len(cost_df.columns) if not cost_df.empty else 0})")
        # 創建一個具有正確結構的空 DataFrame
        cost_df = pd.DataFrame(columns=['Dateonly', 'Cost'])
    
    if not limit_df.empty and len(limit_df.columns) == 5:
        limit_df.columns = ['商品名稱', 'Dateonly', '身份別', '上極限', '下極限']
        ...
        # 創建一個具有正確結構的空 DataFrame
        limit_df = pd.DataFrame(columns=['商品名稱', 'Dateonly', '身份別', '上極限', '下極限'])
    ...
    limit_df.columns = ['商品名稱', 'Dateonly', '外資身份別', '外資上極限', '外資下極限']
    
    limit_df_per.columns = ['自營商商品名稱', 'Dateonly', '自營商身份別', '自營商上極限', '自營商下極限']
...
def update_right_charts(n):
    ...
    for i in range(1,len(df['vol5'])):
        ...
        if df.loc[df.index[i],'vol5'] > df.loc[df.index[i],'vol21'] and df.loc[df.index[i-1],'vol5'] < df.loc[df.index[i-1],'vol21']:
            ...
            df.loc[df.index[i],'vol_sym_v'] = 100 #max(df['vol5'].max(),df['vol21'].max())
        else:
            # 藍色下箭頭
            df.loc[df.index[i],'vol_sym_c'] = '#F99584'
    ...
    fig_right.add_trace(
        go.Scatter(
        x=df.index,
        y=df['vol_sym_v'],  # 使用 vol_sym_v 作為 y 軸位置
        mode='markers',
        marker=dict(
            color=df['vol_sym_c'],    # 顏色
            symbol=df['vol_sym'],     # 符號類型（circle, triangle-up, triangle-down等）
            size=14                   # 符號大小
        ),
        showlegend=False,
        name='成交量信號'
